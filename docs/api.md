...
{
  "combinations": [
    { "id": 7, "name": "Coloris - Bleu nuit", "ean13": "3760123456999", "reference": "RICO-035-BLEU", "quantity": 12 },
    { "id": 8, "name": "Coloris - Rouge",     "ean13": "",              "reference": "RICO-035-ROUGE", "quantity": 4 }
  ],
  "matched_combination": {
    "id": 7,
...
| Champ                            | Type        | Description                                                                 |
|-----------------------------------|-------------|-------------------------------------------------------------------------------|
| `combinations`                    | array       | **v1.10.7.** Toutes les déclinaisons du produit matché, uniquement sur les résultats de `GET /products?barcode=...`. Absent ou `[]` si le produit n'a pas de déclinaison. |
| `combinations[].id`               | int         | `id_product_attribute` de la déclinaison.                                    |
| `combinations[].name`             | string      | Libellé façon core PrestaShop (`attribute_group_lang` + `attribute_lang`).   |
| `combinations[].ean13`            | string      | EAN13 propre à la déclinaison (`product_attribute.ean13`), `""` si absent.   |
| `combinations[].reference`        | string      | Référence propre à la déclinaison (`product_attribute.reference`).           |
| `combinations[].quantity`         | int         | Stock de cette déclinaison (`StockAvailable::getQuantity`).                  |
| `matched_combination`             | object/null | La déclinaison ciblée **sans ambiguïté** par le scan (règles ci-dessous), sinon `null`. |
...
**Règles de `matched_combination`** (dans cet ordre) :
...
> `matched_combination` est toujours `null` et `combinations` est **absent** sur ce endpoint : le détail
> est résolu par `id_product` (pas par `barcode`), donc jamais issu d'un match sur une combinaison. Ces
> deux champs n'ont de sens que sur `GET /products?barcode=...`.
...
| `combination_id`      | int       | **Optionnel (v1.10.7)**. `id_product_attribute` de la déclinaison ciblée. Doit appartenir au produit `{id}` de l'URL. Si fourni avec `ean13`, l'EAN13 est écrit sur **la déclinaison** (`product_attribute.ean13`) au lieu du produit. Sans effet sur les autres champs (`name`, `price_tax_excl`, etc., toujours au niveau produit). |
...
```json
{ "ean13": "3760123456789", "combination_id": 7 }
```
